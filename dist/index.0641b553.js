...
function getWeatherData(city) {
    preEl.textContent = "";
    fetch(`https://api.weatherapi.com/v1/current.json?key=925a0a84b4634431833161143232702&q=${city}&aqi=no`).then((response)=>response.json()).then((data)=>{
        if (data.error) (0, _notiflixNotifyAio.Notify).failure(`${data.error.message}`);
        else preEl.textContent = JSON.stringify(data, null, 2);
    }).catch(()=>{
...
},{}]},["lKzq4","bNKaB"], "bNKaB", "parcelRequirea848")
